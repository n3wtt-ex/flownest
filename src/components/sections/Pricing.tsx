  ...
  return (
    <section id="pricing" aria-labelledby="pricing-heading" className="container py-20">
      ...
      <div className="max-w-2xl mx-auto">
        {plans.map((p) => (
          <Card key={p.name} className={`bg-card/60 border ${p.featured ? "border-primary/50 shadow-glow" : "border-border/60"} dark:bg-gray-800/60 dark:border-gray-700/60`}>
            <CardHeader className="text-center">
              <CardTitle className="text-2xl mb-2 dark:text-white">{p.name}</CardTitle>
              {p.price && <span className="text-3xl font-extrabold dark:text-white">{p.price}</span>}
            </CardHeader>
            <CardContent>
              <ul className="space-y-3 mb-8 text-muted-foreground dark:text-gray-400">
                {p.features.map((f) => (
