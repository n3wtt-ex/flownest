...
export function WorkspaceBoard({ workspace, onUpdateWorkspace }: WorkspaceBoardProps) {
  ...
  // Yeni eklenen animasyon state'i
  ...
  // Güncellenmiş handleStartWorkflow
  ...
  return (
    <>
      <motion.div
        animate={{
          scale: workflowStarted ? 0.3 : 1,
          x: workflowStarted ? window.innerWidth * 0.35 : 0,
          y: workflowStarted ? window.innerHeight * 0.35 : 0,
          transformOrigin: workflowStarted ? 'top left' : 'center center'
        }}
        transition={{ duration: 1.2, ease: 'easeInOut' }}
        className={`w-full h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl border border-slate-700/50 overflow-hidden relative ${workflowStarted ? 'pointer-events-none' : ''}`}
      >
            ...
            <div className="relative h-full flex items-center justify-center">
              ...
              {canStartWorkflow && (
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                  ...
                  <motion.button onClick={handleStartWorkflow}
      ...
      {workflowStarted && (
        ...
        >
          <div className="bg-slate-800 p-8 rounded-xl border border-slate-700 text-center">
            <h2 className="text-white text-2xl font-bold mb-4">🚀 İş Akışı Aktif</h2>
            <p className="text-slate-300">AI agent'lar çalışmaya başladı...</p>
          </div>
        </motion.div>
      )}
    </>
